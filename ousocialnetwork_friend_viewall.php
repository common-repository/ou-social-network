...
$oucode15 = $_POST['nonce'];
if (!wp_verify_nonce($oucode15, '2f6g7h8a3'))
{
    wp_die();
}
if(is_user_logged_in())
{
    ...
    $ousqlfriends = $wpdb->get_results( "SELECT * FROM $ou_sn_table_friends where ousnfriends_status_from ='1' AND ousnfriends_status_to ='1' AND ( (ousnfriends_from  = '$ou_s_n_check_user_idme4') OR (ousnfriends_to = '$ou_s_n_check_user_idme4')) ");
    ...
    { 
        ...
        if($ou_s_n_check_user_idme4 !=$ousnfriends_from_a1)
        {
            $ousnuserfriendsds = $ousnfriends_from_a1;
        }
    
        if($ou_s_n_check_user_idme4 !=$ousnfriends_to_a1)
        {
            $ousnuserfriendsds = $ousnfriends_to_a1;
        }
    
        $ou_sn_table_myprofile8 = $wpdb->prefix . "ousocialnetworkprofile";
        $ou_myprofile_sql7 = $wpdb->get_results( "SELECT * FROM $ou_sn_table_myprofile8 where ousocialnetprofile_id_user = $ousnuserfriendsds AND ousocialnetprofile_firstname !='' AND ousocialnetprofile_lastname !=''");
        ...
        { 
            ...
            function ousnfrremote<?php echo $ousocialnetprofile_id_a7;?>()
            {
                ...
                formData1.append('action', 'ousnformresult16');
                formData1.append('nonce', '<?php echo wp_create_nonce('hjygf45gv');?>');
                jQuery.ajax({
                type: "post",
                url: ousnajaxcode.ousnajax_url,
                data: formData1,
